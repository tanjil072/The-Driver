...
{
	printf("playScoreHome=%d\n",playScoreHome);

	if (mx >= 32 && mx <= 110 && my >= 748 && my <= 758 && PlayRacer == 1 && PlayPolice == 0)
	{
		playScoreHome = 1;
	}
	else if (mx >= 32 && mx <= 110 && my >= 748 && my <= 758 && PlayRacer == 0 && PlayPolice == 1)
	{
		playScoreHome = 1;
	}
	else
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		if (menux >= 73 && menux <= 303 && menuy >= 673 && menuy <= 733 && MenuControl != 1 && MenuAbout != 1 && MenuRank != 1)
				...
				else if (mx >= 73 && mx <= 303 && my >= 483 && my <= 541 && MenuControl != 1 && MenuAbout != 1 && NewGame!=1)
				{
					...
					MenuAbout = 0;
					MenuBack = 0;
					//PlaySound("a", NULL, SND_LOOP | SND_ASYNC);
...
int main()
{
	...
	t=iSetTimer(1500, LoadingPage);
	
	if (introSound == true)
	{
		PlaySound("The driver intro.wav", NULL, SND_ASYNC);
	}
